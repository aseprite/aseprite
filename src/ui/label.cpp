  ...
  switch (msg->type()) {
    case kMouseDownMessage:
      ...
      if (bud && bud->isVisible() && !bud->hasFocus()) {
        ...
        // Perform some automatic actions depending on the widget type.
        switch (bud->type()) {
          case kCheckWidget: {
            bud->setSelected(!bud->isSelected());
          } break;
          case kComboBoxWidget: {
            auto* comboBox = static_cast<ComboBox*>(bud);
            if (!comboBox->isEditable() && comboBox->isClickOpen())
