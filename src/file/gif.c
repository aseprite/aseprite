...
/* XXXXX TODO: find the colors that are used and resort the palette */
...
static int save_GIF(Sprite *sprite)
{
  ...
  for (i = 0; i < sprite->frames; i++) {
    ...
    /* first frame */
    if (i == 0) {
      /* XXXXX TODO: don't use 256 colors, but only as much as needed. */
...
#if 0 /* XXXXX remove this (doesn't work right for sprites with trans-color */
	...
	/* XXXXX TODO: in this case, we should remove this frame, and
    ...
    gif->frames[i].w = w;
    gif->frames[i].h = h;
    gif->frames[i].bitmap_8_bit = calloc(w, h);
