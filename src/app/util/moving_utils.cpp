...
void lockAxisProcess(LockedAxis& lockedAxis, gfx::PointF& delta)
{
  double dx = delta.x;
  double dy = delta.y;
  lockAxisProcess(lockedAxis, dx, dy);
  delta.x = dx;
  delta.y = dy;
}

void lockAxisProcess(LockedAxis& lockedAxis, double& dx, double& dy)
{
  if (lockedAxis == LockedAxis::NONE) {
    ...
    else if (std::abs(dy) > std::abs(dx) * 3) {
