...
static void snap_isometric_line(ToolLoop* loop, Stroke& stroke) {
  // Get line angle
  PointF vto(stroke[1].x - stroke[0].x,
             stroke[1].y - stroke[0].y);
  ...
  if (! int(vto.x) && int(vto.y)) {
    bool lineTool = (string_id_to_brush_type(loop->getTool()->getId()) ==
                     kLineBrushType);
    stroke[0].x -= lineTool;
    stroke[1].x -= lineTool;
  }
  // Diagonal lines for width-to-height ratios greater than 1:1
    ...
    Point delta(std::round(SGN(vto.x) * normal.x * len),
                std::round(SGN(vto.y) * normal.y * len));
