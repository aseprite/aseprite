...
void Extension::addCustomFormat(const CustomFormat& format)
{
  ...
  for (const auto& extension : App::instance()->extensions()) {
    if (m_customFormats.count(format.id) > 0)
      throw base::Exception("Duplicated format ID: " + format.id);

    for (const auto& [id, other] : extension->m_customFormats) {
      for (const auto& otherExt : other.extensions) {
        for (const auto& ourExt : format.extensions) {
          if (base::string_to_lower(otherExt) == base::string_to_lower(ourExt))
  ...
  m_customFormats.try_emplace(format.id, format);
}
...
std::optional<std::string> Extension::getCustomFormatIdForExtension(
  ...
  const CustomFormat::Support support) const
{
  ASSERT(support > 0);
  ...
  for (const auto& [id, format] : m_customFormats) {
    for (const auto& formatExtensionString : format.extensions) {
      if (base::string_to_lower(formatExtensionString) == base::string_to_lower(ext) &&
          format.supports(support)) {
        return id;
      }
    }
  }
...
{
  const auto& format = m_customFormats.at(formatId);
  ASSERT(format.onsaveRef > -1);

  if (!format.supports(CustomFormat::Load))
    return false;
...
bool Extension::saveCustomFormat(const std::string& formatId,
                                 ...
                                 const Sprite* sprite,
                                 const CustomFormatSaveOptions& saveOptions) const
{
  const auto& format = m_customFormats.at(formatId);
  ASSERT(format.onsaveRef > -1);

  if (!format.supports(CustomFormat::Save))
    return false;
...
std::vector<std::string> Extensions::customFormatList(
  const Extension::CustomFormat::Support support) const
{
  ...
  for (auto* ext : m_extensions) {
    ...
    for (const auto& [id, format] : ext->m_customFormats) {
