...
public:
  ...
  obs::safe_signal<void(WebSocketMessageType, const base::buffer&)> Message;

  std::size_t receivedData(char* ptr, const std::size_t bytes)
  {
    const auto* frame = curl_ws_meta(m_curl);
    const base::buffer buffer(ptr, ptr + bytes);
  ...
  {
    if (shouldClose()) {
      Message(WebSocketMessageType::Close, base::buffer());
    ...
    if (!m_reportedOpen) {
      Message(WebSocketMessageType::Open, base::buffer());
      m_reportedOpen = true;
    }
  ...
  void sendData(base::buffer&& data, const bool isBinary = true)
  {
    ASSERT(m_outgoingData.empty() && m_outgoingOffset == 0);
    m_outgoingData = std::move(data);
    m_outgoingOffset = 0;
    m_outgoingIsBinary = isBinary;
    curl_easy_pause(m_curl, CURLPAUSE_CONT);
  }
...
public:
    ...
    , m_connectionRequested(false)
  {
    ...
    // Attempt to detect if registered function has more than 1 parameter,
    // in which case we fall back to the legacy signature.
    if (m_onreceiveRef > 0) {
      lua_rawgeti(L, LUA_REGISTRYINDEX, m_onreceiveRef);
      lua_Debug ar;
      if (!lua_getinfo(L, ">u", &ar))
        throw std::runtime_error("Could not onreceive function arguments");
      m_legacyCallback = (ar.nparams > 1 || ar.isvararg);
    }
  }
  ...
  void threadSocket()
  {
    ...
    m_messageConn = m_conn->Message.connect(
      [this](const WebSocketMessageType type, const base::buffer& buffer) {
        ui::execute_from_ui_thread([this, type, buffer] {
          onMessage(type, reinterpret_cast<const char*>(buffer.data()), buffer.size());
        });
      });
    ...
    if (!m_conn->shouldClose()) {
      if (!result.success) {
        ui::execute_from_ui_thread([this, error = result.errorString] {
          onMessage(WebSocketMessageType::Error, error.data(), error.size());
        });
      }
  ...
  void onMessage(WebSocketMessageType type, const char* data, size_t size)
  {
    ui::assert_ui_thread();
  ...
  void sendText(base::buffer&& data)
  {
    if (!m_conn)
      return;
    const std::scoped_lock lock(m_mutex);
    m_conn->sendData(std::move(data), false);
  }

  void sendBinary(base::buffer&& data)
  {
    if (!m_conn)
      return;
    const std::scoped_lock lock(m_mutex);
...
base::buffer get_send_data(lua_State* L, WebSocket* ws)
{
  if (!ws->isReady())
  ...
  for (int i = 2; i <= argc; i++) {
    ...
    if (buf && bufLen > 0)
      data.insert(data.end(), buf, buf + bufLen);
  }
...
{
  auto* ws = get_obj<WebSocket>(L, 1);
  auto data = get_send_data(L, ws);
  ws->waitForSent();
  ws->sendText(std::move(data));
  return 0;
}
