...
int Plugin_registerFormat(lua_State* L)
{
  auto* plugin = get_obj<Plugin>(L, 1);
  if (!lua_istable(L, 2))
    return luaL_error(L,
                      "plugin:registerFormat() must be called with a table as its first argument");

  Extension::CustomFormat format;
  ...
  plugin->ext->addCustomFormat(format);
  return 0;
}
...
const luaL_Reg Plugin_methods[] = {
  ...
  { "registerFormat",   Plugin_registerFormat   },
  { nullptr,            nullptr                 }
};
