  ...
  , m_scaryAllowCooldown(4000) // TODO: Configurable?
{
  ...
  if (permission_is_scary(permission)) {
    ...
    if (m_scaryAllowCooldown > 0) {
      ...
      auto reset = [this, original] {
        allow()->setText(original);
        allow()->setEnabled(true);
        m_timer.stop();
      };

      // Make it so that any interaction with the remember checkboxes will accelerate
      // the cooldown and enable the button.
      remember()->Click.connect(reset);
      rememberFile()->Click.connect(reset);
      rememberDirectory()->Click.connect(reset);
      rememberExtra()->Click.connect(reset);

      auto tick = [this, original, reset] {
