...
{
  // Using clip::has(clip::empty_format()) had inconsistent results, might as well avoid false positives
  // by just checking the two formats we support
  lua_pushboolean(
    L, !clip::has(clip::image_format()) && !clip::has(clip::text_format())
  );
  return 1;
}
