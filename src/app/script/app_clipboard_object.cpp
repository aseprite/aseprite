...
#define CLIPBOARD_GATE(mode) \
  if (!ask_access(L, nullptr, mode, ResourceType::Clipboard)) \
    return luaL_error(L, "the script doesn't have %s access to the clipboard", (mode) == FileAccessMode::Read ? "read" : "write");
...
int Clipboard_get_image(lua_State* L)
{
  ...
  const bool result = app::Clipboard::instance()->getNativeBitmap(
    &image, &mask, &palette, &tileset
  );
...
int Clipboard_get_content(lua_State* L)
{
  ...
  doc::Palette* palette = nullptr;
  doc::Tileset* tileset = nullptr;
  const bool bitmapResult = app::Clipboard::instance()->getNativeBitmap(
    &image, &mask, &palette, &tileset
  );
...
int Clipboard_set_content(lua_State* L)
{
  ...
  if (type != LUA_TNIL) {
    ...
    if (!image) return luaL_error(L, "invalid image provided");
  }
  lua_pop(L, 1);
  ...
  if (type != LUA_TNIL) {
    ...
    if (!palette) return luaL_error(L, "invalid palette provided");
  }
  lua_pop(L, 1);
  ...
  if (type != LUA_TNIL) {
    ...
    if (!tileset) return luaL_error(L, "invalid tileset provided");
  }
  lua_pop(L, 1);
  ...
  if (image && !app::Clipboard::instance()->setNativeBitmap(image, mask, palette ? palette : get_current_palette(), tileset, image ? image->maskColor() : -1))
...
const luaL_Reg Clipboard_methods[] = { { "clear", Clipboard_clear },
                                       { nullptr, nullptr } };
