...
std::string read_code(const std::string& filename, const size_t limit = 0)
{
  std::ifstream stream(FSTREAM_PATH(filename), std::ios::binary);

  // Match the behavior of Lua's skipcomment and skipBOM, which if we don't will lead us to fail to
  // detect bytecode files, since it first attempts to skip a valid UTF BOM, and then any line
  // starting with # and THEN it checks for the LUA_SIGNATURE
  int c = stream.get();
  if (c == 0xEF) {
    const int c2 = stream.get();
    const int c3 = stream.get();
    if (c2 == 0xBB && c3 == 0xBF) {
      c = stream.get();
    }
    else {
  ...
  if (limit > 0) {
    ...
    stream.read(&content[0], limit);
    content.resize(stream.gcount());
  }
  else {
    content.assign((std::istreambuf_iterator(stream)), std::istreambuf_iterator<char>());
  }
  return content;
}
...
Engine::Engine(const std::string& extensionName)
  : L(nullptr)
  , m_extensionName(extensionName)
  , m_printEvalResult(false)
  , m_returnCode(0)
  , m_objectTracker(0)
{
...
int Engine::lua_dofile()
{
  ...
  const std::string code = read_code(filename, 4);
  if (code.substr(0, 4) == LUA_SIGNATURE)
    accessGate(Permission::Bytecode);
...
bool Engine::requestAccess(const Permission permission, const std::string& match)
{
  ...
  try {
    opt = permission_supports_matching(permission) ? storage->getMatch(script, permission, match) :
                                                     storage->get(script, permission);
  }
  catch (const std::exception& e) {
    LOG(WARNING,
  ...
  try {
    ...
    switch (remember) {
      ...
      case PermissionDialog::Remember::FullAccess: storage->setFullAccess(script, true); break;
      case PermissionDialog::Remember::Permission: storage->set(script, permission, allow); break;
      case PermissionDialog::Remember::AllOfType:  {
        storage->setForMatch(script, permission, "*", allow);
        break;
      }
      case PermissionDialog::Remember::Match: {
        ...
        storage->setForMatch(script, permission, safeMatch, allow);
        break;
      }
      case PermissionDialog::Remember::Directory: {
        const std::string dirMatch = base::join_path(base::get_file_path(match), "*");
        storage->setForMatch(script, permission, dirMatch, allow);
        break;
      }
    }
    return allow;
    ...
    LOG(ERROR, "Permission dialog error: %s\n", e.what());
  }
...
bool Engine::evalCode(const std::string& code, const std::string& name)
{
  bool ok = true;
  try {
    if (code.substr(0, 4) == LUA_SIGNATURE && !requestAccess(Permission::Bytecode))
      return false;
...
bool Engine::evalFile(const std::string& filename, const Params& params)
{
  ...
  const bool result = evalCode(read_code(filename), "@" + absFilename);
  return result;
}
