...
int AppNet_fetch(lua_State* L)
{
  ...
  params.headers.setHeader(
    "User-Agent",
    fmt::format("Mozilla/5.0 (compatible; {}/{}; +{}api/app_net#appnetfetch)",
                get_app_name(),
                get_app_version(),
                get_app_url()));
  ...
  if (lua_istable(L, 1)) {
    ...
    if (type == LUA_TSTRING || type == LUA_TNUMBER) {
      ...
      size_t len;
      // Automatically gets stringified by lua through JsonObj_tostring()
  ...
  if (methodString == "get")
    ...
    params.method = net::HttpRequest::Method::DELETE;
  else
    return luaL_error(L,
                      "invalid 'method', must be one of GET, POST, PUT, PATCH, DELETE or OPTIONS");
