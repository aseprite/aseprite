...
std::optional<bool> PermissionStorage::get(const std::string& script,
...
std::optional<bool> PermissionStorage::getMatch(const std::string& script,
...
void PermissionStorage::set(const std::string& script, const Permission permission, bool value)
{
  ASSERT(!permission_supports_matching(permission));
  ASSERT(permission != Permission::Unknown);
...
void PermissionStorage::setForMatch(const std::string& script,
                                    const Permission permission,
...
bool PermissionStorage::getFullAccess(const std::string& script) const
{
  if (!m_json.contains(script))
    return false;
...
void PermissionStorage::setFullAccess(const std::string& script, const bool access)
{
...
void PermissionStorage::reset(const std::string& script)
{
  try {
    ...
    LOG(ERROR, "Parsing error while resetting permissions with error '%s'\n", e.what());
    m_json = json::object();
  }
  flush();
}
...
void PermissionStorage::reset(const std::string& script, Permission permission)
{
  ...
  try {
    ...
    LOG(ERROR,
        "Error while resetting permission '%s': %s\n",
        permission_to_string(permission).data(),
        e.what());
  }
}
...
void PermissionStorage::flush()
{
  ...
  execute_now_or_enqueue([this] {
    ...
    if (out) {
      ...
      LOG(ERROR, "Failed to write permissions to '%s'\n", m_path.c_str());
    }
  });
}
