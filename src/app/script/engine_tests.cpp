...
TEST(Engine, Security)
{
  ...
  storage.setForMatch(lua.filename, Permission::Execute, "cd", true);

  engine->setPrintEvalResult(true);
  engine->evalUserFile(lua.filename);
  EXPECT_TRUE(err.empty());

  storage.setForMatch(lua.filename, Permission::Execute, "cd", false);
  engine->evalUserFile(lua.filename);
...
TEST(Engine, TempFiles)
{
  ...
  storage.set(lua.filename, Permission::TemporaryFile, true);
