...
public:
  ...
  struct CustomFormatSaveOptions {
    gfx::Size canvasSize;
    gfx::Rect bounds;
  ...
  std::optional<std::string> getCustomFormatIdForExtension(const std::string& ext,
                                                           CustomFormat::Support support) const;
...
private:
  ...
  std::unordered_map<std::string, CustomFormat> m_customFormats;
#endif
...
public:
  ...
  std::vector<std::string> customFormatList(
    Extension::CustomFormat::Support support = Extension::CustomFormat::Support::Full) const;
#endif
