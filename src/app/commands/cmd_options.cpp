...
public:
  ...
  void loadFromPreferences()
  {
    ...
    if (App::instance()->isDevMode())
      ...
      undoSaveHistoryOptions()->setVisible(false);
...
private:
  ...
  void loadPermissions()
  {
    ...
    try {
      ...
      for (const auto& script : storage->scripts()) {
        ...
        if (storage->getFullAccess(script)) {
        ...
        for (const auto& permission : storage->stored(script)) {
          ...
          if (script::permission_supports_matching(permission)) {
            ...
            const bool allowed = storage->get(script, permission).value_or(false);
            permNode = std::make_unique<PermTreeNode>(PermTreeNode::Type::Permission,
                                                      permName,
  ...
  void onPermissionsRightClickItem()
  {
    ...
    if (hasPath) {
      ...
      openFolder.Click.connect([path] { base::launcher::open_folder(base::get_file_path(path)); });
    }
    if (canRevoke)
  ...
  void onAllowDenyPermission(bool value)
  {
    ...
    switch (type) {
      case PermTreeNode::Type::Permission: {
        ...
        storage->set(script, node->permission(), value);
        break;
      }
      case PermTreeNode::Type::Match: {
        ...
        storage->setForMatch(script, permission, node->match(), value);
        break;
      }
      default: return;
    }
  ...
  void onRevokePermission()
  {
    ...
    switch (type) {
      ...
      case PermTreeNode::Type::FullAccess: {
        const auto& script = static_cast<PermTreeNode*>(node->parent())->script();
        storage->setFullAccess(script, false);
        break;
      }
