...
static ImageRef loadPbmHeader(std::ifstream& istream, FileOp* fop)
{
  ...
  ImageRef image = fop->sequenceImageToLoad(IMAGE_BITMAP, width, height);
  return image;
}
                                                              ...
                                                              PnmFormat format)
{
  ...
  std::optional<uint16_t> depth = 0;
  std::optional<uint16_t> maxval = 0;
  int tuplTypeLen = 0;
  char tuplType[21];
  ...
  while (true) {
    ...
    char cmd[9];
    int cmdLen = 0;
    for (int cmdLen = 0; cmdLen < 9; ++cmdLen) {
...
endhdr:
  ...
  switch (tuplType[0]) {
    case 'B':
      ...
      if (depth != 1 + (knownType == BLACKANDWHITE ? 1 : 2)) {
    ...
    case 'G':
      ...
      if (depth != 1 + (knownType == GRAYSCALE ? 1 : 2)) {
        fop->setError("PAM image had TUPLTYPE = %s but DEPTH = %d\n", tuplType, depth);
        return std::tuple(nullptr, RGB, 0);
      }
