...
#include "app/file/undo_history_io.h"
...
void load_undo_history(FileOp* fop)
{
  ...
  while (fi.ok()) {
    ...
    if (tok == "Sp") {
      ...
      if (!lastCel)
        throw std::runtime_error("undo history: unexpected cel data before cel");
  ...
  while (fi.ok()) {
    ...
    if (auto* cmdTx = dynamic_cast<cmd::CmdTransaction*>(cmd))
      undo->add(cmdTx, (serial.beforeCurState() ? DocUndo::AddIn::Undo : DocUndo::AddIn::Redo));
    else
...
void save_undo_history(FileOp* fop)
{
  ...
  // Write IDs map
  {
    ...
    std::function<void(const Layer*)> writeLayerIds = [&](const Layer* layer) {
      ...
      for (const Cel* cel : layer->cels()) {
        ...
        if (cel->data())
          fprintf(handle.get(), " Cd %d", cel->dataId());
        if (cel->image())
  ...
  // If the current state is the initial point of the undo history.
  if (undo->currentState() == nullptr)
    serial.curStateMark();
  ...
  for (const auto* state = undo->firstState(); state; state = state->next()) {
    ...
    // The mark is going to the next "Tx"
