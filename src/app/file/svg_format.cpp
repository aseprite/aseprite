...
bool SvgFormat::onSave(FileOp* fop)
{
  ...
  auto printRect = [f](int x, int y, int width, int height, int r, int g, int b, int a, int pxScale) {
    fprintf(f,
  ...
  auto mergePixels = [&](auto getPixelColor, auto getColorComponents) {
    std::vector<std::vector<bool>> processed(image->height(), std::vector<bool>(image->width(), false));
    ...
    for (y = 0; y < image->height(); y++) {
      for (x = 0; x < image->width(); x++) {
        ...
        for (int w = x; w < image->width(); w++) {
          if (processed[y][w]) break;
          auto [checkR, checkG, checkB, checkA, skip] = getPixelColor(w, y);
          if (skip || checkR != pixelR || checkG != pixelG || checkB != pixelB || checkA != pixelA) break;
          maxWidth = w - x + 1;
        }
  ...
  switch (image->pixelFormat()) {
    case IMAGE_RGB: {
      auto getPixelColor = [&](int px, int py) -> std::tuple<int, int, int, int, bool> {
        ...
        alpha = rgba_geta(c);
        if (alpha == 0x00) return {0, 0, 0, 0, true};
        return {rgba_getr(c), rgba_getg(c), rgba_getb(c), alpha, false};
      };
      mergePixels(getPixelColor, nullptr);
      break;
    }
    ...
    case IMAGE_INDEXED: {
      ...
      auto getPixelColor = [&](int px, int py) -> std::tuple<int, int, int, int, bool> {
        ...
        if (c == mask_color) return {0, 0, 0, 0, true};
        return {image_palette[c][0] & 0xff, image_palette[c][1] & 0xff, 
                image_palette[c][2] & 0xff, image_palette[c][3] & 0xff, false};
      };
      mergePixels(getPixelColor, nullptr);
      break;
    }
