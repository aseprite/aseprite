...
void TextEdit::onPaint(PaintEvent& ev)
{
  ...
  selectedTextPaint.color(theme->colors.selectedText());
  ...
  for (const auto& line : m_lines) {
    ...
    if (!skip) {
      ...
      if (!selectionRect.isEmpty()) {
        g->fillRect(theme->colors.selected(), selectionRect);
  ...
  if (m_drawCaret)
    g->drawRect(theme->colors.text(), m_caretRect);
...
{
  Menu menu;
  MenuItem cutItem(Strings::textedit_cut());
  MenuItem copyItem(Strings::textedit_copy());
  MenuItem pasteItem(Strings::textedit_paste());
  MenuItem selectAllItem(Strings::textedit_select_all());

  cutItem.processMnemonicFromText();
  copyItem.processMnemonicFromText();
  pasteItem.processMnemonicFromText();
  selectAllItem.processMnemonicFromText();
  ...
  cutItem.setEnabled(!m_selection.isEmpty());
  copyItem.setEnabled(!m_selection.isEmpty());

  cutItem.Click.connect(&TextEdit::cut, this);
  copyItem.Click.connect(&TextEdit::copy, this);
  pasteItem.Click.connect(&TextEdit::paste, this);
  selectAllItem.Click.connect(&TextEdit::selectAll, this);
...
void TextEdit::ensureCaretVisible()
{
  ...
  const auto* theme = skin::SkinTheme::get(this);
  const int scrollBarWidth = theme->dimensions.scrollbarSize();
