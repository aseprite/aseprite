  ...
  TimelineButtons() : ButtonSet(2) {
    ...
    m_timelinePosConn = timelinePosOption.AfterChange.connect(
      [this](gen::TimelinePosition position){
        setSelectedButtonFromTimelinePosition(position);
      });
    ...
    for (auto item : m_comboBox) {
      if (auto layoutItem = dynamic_cast<LayoutItem*>(item)) {
        if (layoutItem->layout() &&
            layoutItem->layout()->name() == layout->name()) {
...
void LayoutSelector::onAnimationFrame()
{
  switch (animation()) {
    ...
    case ANI_COLLAPSING: {
      ...
      m_comboBox.setSizeHint(
        gfx::Size(int(inbetween(m_startSize.w, m_endSize.w, t)),
