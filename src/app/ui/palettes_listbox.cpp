...
public:
  ...
  {
    if (m_initialized) {
      // Replace the pinned button with a new one.
      // TODO: Would it be faster/better to replace the part & tooltip?
      auto* oldButton = m_hbox.lastChild();
      m_hbox.removeChild(oldButton);
      m_tooltips->removeTooltipFor(oldButton);
      oldButton->deferDelete();
    }

    auto* pinnedButton = new IconButton(isPinned ? SkinTheme::instance()->parts.pinned() :
