...
//
// Other key context are just temporal in the Editor, like a mouse
// drag operation when we are transforming the
// selection. E.g. TranslatingSelection, ScalingSelection, etc.
enum class KeyContext {

  // Special context to define keys. This is not be used as "current
  ...
  // The user has moved the selection and didn't still dropped the
  // pixels (the user can drop or undo the transformation).
  Transformation,
};
