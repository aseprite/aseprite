...
Timeline::Hit::Hit(
  int part, layer_t layer, frame_t frame, ObjectId tag, int band)
  : part(part)
  , layer(layer)
  , frame(frame)
  , tag(tag)
...
{
  return part != other.part || layer != other.layer || frame != other.frame ||
         tag != other.tag || band != other.band;
}
...
void Timeline::setZoom(const double zoom)
{
  ...
  m_thumbnailsOverlayDirection =
    gfx::Point(int(frameBoxWidth() * 1.0), int(frameBoxWidth() * 0.5));
...
{
  setZoomAndUpdate(
    docPref().thumbnails.enabled() ? docPref().thumbnails.zoom() : 1.0, false);
}
...
void Timeline::updateUsingEditor(Editor* editor)
{
  ...
  m_onionskinConn =
    docPref.onionskin.AfterChange.connect([this] { invalidate(); });
  ...
  switch (msg->type()) {
    ...
    case kTimerMessage:
      if (static_cast<TimerMessage*>(msg)->timer() == &m_clipboard_timer) {
        ...
        Clipboard::instance()->getDocumentRangeInfo(&clipboard_document,
                                                    &clipboard_range);
        ...
        if (isVisible() && m_document && m_document == clipboard_document) {
          ...
          invalidateRect(
            gfx::Rect(getRangeBounds(clipboard_range)).offset(origin()));
    ...
    case kMouseDownMessage: {
      ...
      switch (m_hot.part) {
        ...
        case PART_HEADER_EYE: {
          ...
          for (Layer* topLayer : m_sprite->root()->layers()) {
            if (topLayer->isVisible() != newVisibleState) {
              m_document->setLayerVisibilityWithNotifications(topLayer,
                                                              newVisibleState);
        ...
        case PART_ROW_TEXT: {
          ...
          const bool selectLayer =
            (mouseMsg->left() || !isLayerActive(m_clk.layer));
          const bool selectLayerInCanvas =
            (m_clk.layer != -1 && mouseMsg->left() &&
          ...
          if (selectLayerInCanvas) {
            ...
            handleRangeMouseDown(
              msg, Range::kLayers, m_rows[m_clk.layer].layer(), m_frame);
          ...
          if (validLayer(m_clk.layer)) {
            ...
            if (msg->altPressed()) {
              ...
              // In other case, hide everything
                ...
                for (Row& row : m_rows) {
                  Layer* l = row.layer();
                  l->switchFlags(LayerFlags::Internal_WasVisible,
                                 l->isVisible());
                  m_document->setLayerVisibilityWithNotifications(l, false);
                }
              }
        ...
        case PART_ROW_PADLOCK_ICON:
          if (validLayer(m_hot.layer)) {
            ...
            setLayerEditableFlag(m_clk.layer,
                                 m_state == STATE_UNLOCKING_LAYERS);
          }
          break;
        ...
        case PART_ROW_CONTINUOUS_ICON:
          if (validLayer(m_hot.layer)) {
            ...
            if (layer->isImage()) {
              ...
              setLayerContinuousFlag(
                m_clk.layer, m_state == STATE_ENABLING_CONTINUOUS_LAYERS);
            }
              ...
              setLayerCollapsedFlag(m_clk.layer,
                                    m_state == STATE_COLLAPSING_LAYERS);
        ...
        case PART_CEL: {
          ...
          if (selectCelInCanvas) {
            ...
            if (selectCel) {
              ...
              handleRangeMouseDown(
                msg, Range::kCels, m_rows[m_clk.layer].layer(), m_clk.frame);
            }
        ...
        case PART_RANGE_OUTLINE:
          ...
          if (m_range.type() == Range::kCels) {
            ...
            if (m_range.layers() > 0) {
              ...
              if (selectedLayersBounds(
                    selectedLayers(), &layerFirst, &layerLast)) {
        ...
        switch (m_state) {
          ...
          case STATE_UNLOCKING_LAYERS:
            ...
            if (hit.part == PART_ROW_PADLOCK_ICON) {
              setLayerEditableFlag(hit.layer,
                                   m_state == STATE_UNLOCKING_LAYERS);
            }
            break;
          ...
          case STATE_DISABLING_CONTINUOUS_LAYERS:
            ...
            if (hit.part == PART_ROW_CONTINUOUS_ICON) {
              setLayerContinuousFlag(
                hit.layer, m_state == STATE_ENABLING_CONTINUOUS_LAYERS);
            }
            break;
          ...
          case STATE_COLLAPSING_LAYERS:
            m_clk = hit;
            if (hit.part == PART_ROW_CONTINUOUS_ICON) {
              setLayerCollapsedFlag(hit.layer,
                                    m_state == STATE_COLLAPSING_LAYERS);
              updateByMousePos(msg, mousePosInClientBounds());
            }
            break;
              ...
              switch (m_state) {
                case STATE_RESIZING_TAG_LEFT:
                  m_resizeTagData.from =
                    std::clamp(hit.frame, 0, tag->toFrame());
                  break;
                case STATE_RESIZING_TAG_RIGHT:
                  m_resizeTagData.to = std::clamp(
                    hit.frame, tag->fromFrame(), m_sprite->lastFrame());
                  break;
              }
              invalidate();
            }
        ...
        switch (m_hot.part) {
          ...
          case PART_TAG: {
            ...
            if (tag) {
              Params params;
              params.set("id",
                         base::convert_to<std::string>(tag->id()).c_str());
        ...
        switch (m_state) {
          case STATE_MOVING_RANGE:
            if (m_dropRange.type() != Range::kNone) {
              dropRange(is_copy_key_pressed(mouseMsg) ? Timeline::kCopy :
                                                        Timeline::kMove);
            }
            break;
          ...
          case STATE_RESIZING_TAG_RIGHT: {
            ...
            if (tag) {
              if ((m_state == STATE_RESIZING_TAG_LEFT &&
                  ...
                  (m_state == STATE_RESIZING_TAG_RIGHT &&
                   tag->toFrame() != m_resizeTagData.to)) {
                try {
                  ...
                  tx(new cmd::SetTagRange(
                    tag,
                    (m_state == STATE_RESIZING_TAG_LEFT ? m_resizeTagData.from :
                                                          tag->fromFrame()),
                    (m_state == STATE_RESIZING_TAG_RIGHT ? m_resizeTagData.to :
                                                           tag->toFrame())));
                  tx.commit();
                }
    ...
    case kDoubleClickMessage:
      switch (m_hot.part) {
        case PART_ROW_TEXT: {
          Command* command =
            Commands::instance()->byId(CommandId::LayerProperties());
        ...
        case PART_HEADER_FRAME: {
          Command* command =
            Commands::instance()->byId(CommandId::FrameProperties());
          Params params;
        ...
        case PART_CEL: {
          Command* command =
            Commands::instance()->byId(CommandId::CelProperties());
      ...
      if (m_document) {
        ...
        if (msg->ctrlPressed() ||  // TODO configurable
          ...
          if (!precise) {
            ...
            if (delta.x ==
                  0 &&  // On macOS shift already changes the wheel axis
                msg->shiftPressed()) {
...
void Timeline::onPaint(ui::PaintEvent& ev)
{
  ...
  try {
    ...
    for (layer = lastLayer; layer >= firstLayer; --layer) {
      ...
      for (frame = firstFrame; frame <= lastFrame; ++frame) {
        Cel* cel =
          (data.it != data.end && (*data.it)->frame() == frame ? *data.it :
                                                                 nullptr);
...
void Timeline::onAfterLayerVisibilityChange(DocEvent& ev)
{
  ...
  if (layerIdx >= 0)
    invalidateRect(
      getPartBounds(Hit(PART_ROW_EYE_ICON, layerIdx)).offset(origin()));
}
...
{
  layer_t i =
    lastLayer() - ((viewScroll().y + getCelsBounds().h) / layerBoxHeight());
                       ...
                       const DrawCelData* data)
{
  ...
  if (layer == m_layer && frame == m_frame)
    drawPart(g,
             ...
             m_range.enabled() ? styles.timelineFocusedCel() :
                                 styles.timelineSelectedCel(),
             false,
             is_hover,
             true);
  else if (m_range.enabled() && is_active)
    drawPart(
      g, bounds, nullptr, styles.timelineSelectedCel(), false, is_hover, true);
  else
    drawPart(
      g, bounds, nullptr, styles.timelineBox(), is_loosely_active, is_hover);
  ...
  if ((docPref().thumbnails.enabled() && m_zoom > 1) && image) {
    gfx::Rect thumb_bounds =
      gfx::Rect(bounds).shrink(skinTheme()->calcBorder(this, style));

    if (!thumb_bounds.isEmpty()) {
      if (os::SurfaceRef surface =
            thumb::get_cel_thumbnail(cel, thumb_bounds.size())) {
  ...
  if (data && data->activeIt != data->end)
    drawCelLinkDecorators(
      g, full_bounds, cel, frame, is_loosely_active, is_hover, data);

  // Draw 'z' if this cel has a custom z-index (non-zero)
  if (cel && cel->zIndex() != 0) {
    drawPart(
      g, bounds, nullptr, styles.timelineZindex(), is_loosely_active, is_hover);
  }
}
...
void Timeline::drawCelOverlay(ui::Graphics* g)
{
  ...
  gfx::Rect rc =
    m_sprite->bounds().fitIn(gfx::Rect(m_thumbnailsOverlayBounds).shrink(1));
                                     ...
                                     const DrawCelData* data)
{
  ...
  bool left =
    (data->firstLink != data->end ? frame > (*data->firstLink)->frame() :
                                    false);
  bool right =
    (data->lastLink != data->end ? frame < (*data->lastLink)->frame() : false);
...
void Timeline::drawTags(ui::Graphics* g)
{
  ...
  if (m_hot.band >= 0 && m_tagBands > 1 && m_tagFocusBand < 0) {
    gfx::Rect bandBounds =
      getPartBounds(Hit(PART_TAG_BAND, -1, 0, doc::NullId, m_hot.band));
  ...
  for (int pass = 0; pass < passes; ++pass) {
    for (Tag* tag : m_sprite->tags()) {
      ...
      gfx::Rect bounds1 =
        getPartBounds(Hit(PART_HEADER_FRAME, firstLayer(), fromFrame));
      gfx::Rect bounds2 =
        getPartBounds(Hit(PART_HEADER_FRAME, firstLayer(), toFrame));
      ...
      if ((m_clk.part == PART_TAG_LEFT && m_clk.tag == tag->id()) ||
          (m_clk.part != PART_TAG_LEFT && m_hot.part == PART_TAG_LEFT &&
           m_hot.tag == tag->id())) {
        ...
        drawTagBraces(
          ...
          bg,
          bounds,
          gfx::Rect(bounds.x, bounds.y, frameBoxWidth() / 2, bounds.h));
      }
        ...
        drawTagBraces(g,
                      bg,
                      bounds,
                      gfx::Rect(bounds.x2() - frameBoxWidth() / 2,
                                bounds.y,
                                frameBoxWidth() / 2,
                                bounds.h));
      }
  ...
  if (m_hot.band >= 0 && m_tagBands > 1) {
    ...
    theme->paintWidgetPart(
      g, styles.timelineSwitchBandButton(), butBounds, info);
  }
}
  ...
  switch (hit.part) {
    ...
    case PART_HEADER_FRAME:
      return gfx::Rect(bounds.x + separatorX() + m_separator_w - 1 +
                         frameBoxWidth() * std::max(firstFrame(), hit.frame) -
                         viewScroll().x,
                       bounds.y + y,
                       frameBoxWidth(),
                       headerBoxHeight());
      ...
      if (validLayer(hit.layer)) {
        return gfx::Rect(bounds.x,
                         bounds.y + y + headerBoxHeight() +
                           layerBoxHeight() * (lastLayer() - hit.layer) -
                           viewScroll().y,
                         separatorX(),
                         layerBoxHeight());
      }
    ...
    case PART_TAG_BAND:
      return gfx::Rect(bounds.x + separatorX() + m_separator_w - 1,
                       bounds.y + (m_tagFocusBand < 0 ?
                                     oneTagHeight() * std::max(0, hit.band) :
                                     0),
                       bounds.w - separatorX() - m_separator_w + 1,
                       oneTagHeight());
...
void Timeline::regenerateRows()
{
  ...
  size_t i = 0;
  for_each_expanded_layer(
    m_sprite->root(), [&i, this](Layer* layer, int level, LayerFlags flags) {
...
void Timeline::updateScrollBars()
{
  ...
  ui::setup_scrollbars(
    getScrollableSize(), m_viewportArea, *this, m_hbar, m_vbar);
...
Timeline::Hit Timeline::hitTest(ui::Message* msg, const gfx::Point& mousePos)
{
  ...
  if (m_clk.part == PART_SEPARATOR) {
    ...
    hit.frame = frame_t((mousePos.x - separatorX() - m_separator_w + scroll.x) /
                        frameBoxWidth());
    ...
    // Is the mouse on the frame tags area?
      ...
      if (hit.part == PART_NOTHING) {
        if (m_tagFocusBand < 0) {
          ...
          gfx::Rect bounds = getPartBounds(Hit(
            PART_TAG_SWITCH_BAND_BUTTON, 0, 0, doc::NullId, m_tagFocusBand));
          if (bounds.contains(mousePos)) {
            hit.part = PART_TAG_SWITCH_BAND_BUTTON;
        ...
        for (Tag* tag : m_sprite->tags()) {
          ...
          // Check if we are in the left/right handles to resize the tag
            ...
            gfx::Rect bounds2 = getPartBounds(
              Hit(PART_HEADER_FRAME, firstLayer(), tag->toFrame()));
            gfx::Rect bounds = bounds1.createUnion(bounds2);
            bounds.h = bounds.y2() - tagBounds.y2();
            bounds.y = tagBounds.y2();

            gfx::Rect bandBounds =
              getPartBounds(Hit(PART_TAG_BAND, 0, 0, doc::NullId, band));

            const int fw = frameBoxWidth() / 2;
            if (gfx::Rect(bounds.x2() - fw, bounds.y, fw, bounds.h)
                  .contains(mousePos)) {
            ...
            else if (gfx::Rect(bounds.x, bounds.y, fw, bounds.h)
                       .contains(mousePos)) {
      ...
      if (hit.part == PART_NOTHING) {
        if (m_tagFocusBand < 0) {
          ...
          gfx::Rect bounds = getPartBounds(
            Hit(PART_TAG_BAND, 0, 0, doc::NullId, m_tagFocusBand));
          if (bounds.contains(mousePos)) {
            hit.part = PART_TAG_BAND;
            hit.band = m_tagFocusBand;
          }
    ...
    // Is the mouse on a layer's label?
      ...
      else if (getPartBounds(Hit(PART_ROW_PADLOCK_ICON, hit.layer))
                 .contains(mousePos))
        hit.part = PART_ROW_PADLOCK_ICON;
      else if (getPartBounds(Hit(PART_ROW_CONTINUOUS_ICON, hit.layer))
                 .contains(mousePos))
...
Timeline::Hit Timeline::hitTestCel(const gfx::Point& mousePos)
{
  ...
  hit.layer = lastLayer() - ((mousePos.y - top - headerBoxHeight() + scroll.y) /
                             layerBoxHeight());
...
void Timeline::updateStatusBar(ui::Message* msg)
{
  ...
  if (m_state == STATE_MOVING_RANGE && msg) {
    ...
    switch (m_range.type()) {
      ...
      case Range::kFrames:
        if (validFrame(m_hot.frame)) {
          if (m_dropTarget.hhit == DropTarget::Before) {
            sb->setStatusText(0,
                              fmt::format("{} before frame {}",
                                          verb,
                                          int(m_dropRange.firstFrame() + 1)));
            return;
          }
          else if (m_dropTarget.hhit == DropTarget::After) {
            sb->setStatusText(0,
                              fmt::format("{} after frame {}",
                                          verb,
                                          int(m_dropRange.lastFrame() + 1)));
            return;
          }
        }
        break;
      ...
      case Range::kLayers: {
        ...
        layer_t lastLayer;
        if (!selectedLayersBounds(
              m_dropRange.selectedLayers(), &firstLayer, &lastLayer))
          break;
        ...
        Layer* layer =
          (validLayer(layerIdx) ? m_rows[layerIdx].layer() : nullptr);
        if (layer) {
          switch (m_dropTarget.vhit) {
            case DropTarget::Bottom:
              sb->setStatusText(
                0, fmt::format("{} below layer '{}'", verb, layer->name()));
              return;
            case DropTarget::Top:
              sb->setStatusText(
                0, fmt::format("{} above layer '{}'", verb, layer->name()));
              return;
            case DropTarget::FirstChild:
              sb->setStatusText(0,
                                fmt::format("{} as first child of group '{}'",
                                            verb,
                                            layer->name()));
              return;
          }
    ...
    Layer* layer =
      (validLayer(m_hot.layer) ? m_rows[m_hot.layer].layer() : nullptr);

    switch (m_hot.part) {
      case PART_HEADER_ONIONSKIN: {
        sb->setStatusText(
          0,
          fmt::format("Onionskin is {}",
      ...
      case PART_TAG: {
        ...
        updateStatusBarForFrame(
          frame, m_hot.getTag(), (layer ? layer->cel(frame) : nullptr));
        return;
      }
    }
  }
                                       ...
                                       const Cel* cel)
{
  ...
  if (firstFrame != lastFrame) {
    buf +=
      fmt::format(" [{}...{}]", int(base + firstFrame), int(base + lastFrame));
  }
...
void Timeline::showCel(layer_t layer, frame_t frame)
{
  ...
  gfx::Rect celBounds(viewport.x + frameBoxWidth() * frame - scroll.x,
                      viewport.y + layerBoxHeight() * (lastLayer() - layer) -
                        scroll.y,
                      frameBoxWidth(),
                      layerBoxHeight());
...
{
  if (m_sprite) {
    return gfx::Size(m_sprite->totalFrames() * frameBoxWidth() +
                       getCelsBounds().w / 2,
...
bool Timeline::isCelLooselyActive(const layer_t layerIdx,
...
void Timeline::dropRange(DropOp op)
{
  ...
  try {
    ...
    if (copy)
      newFromRange =
        copy_range(m_document, m_range, dropRange, place, tagsHandling);
    else
      newFromRange =
        move_range(m_document, m_range, dropRange, place, tagsHandling);
    ...
    if (m_range.type() == Range::kFrames && m_sprite &&
        !m_sprite->tags().empty()) {
...
void Timeline::updateDropRange(const gfx::Point& pt)
{
  ...
  switch (m_range.type()) {
    ...
    case Range::kLayers:
      m_dropRange.clearRange();
      if (!m_rows.empty()) {
        m_dropRange.startRange(
          m_rows[m_hot.layer].layer(), m_hot.frame, m_range.type());
  ...
  // Special drop target for expanded groups
           ...
           static_cast<LayerGroup*>(m_rows[m_hot.layer].layer())
             ->isExpanded()) {
    m_dropTarget.vhit = DropTarget::FirstChild;
  }
  else {
...
{
  return font()->height() + 2 * ui::guiscale() +
