...
class FilenameFieldObserver : public obs::observers<FilenameField> {
public:
  static FilenameFieldObserver& instance()
  {
    static FilenameFieldObserver singleton;
    return singleton;
  }

  FilenameFieldObserver(const FilenameFieldObserver&) = delete;
  void operator=(const FilenameFieldObserver&) = delete;

private:
  FilenameFieldObserver()
  {
    Preferences::instance().general.editFullPath.AfterChange.connect(
      [this] { notify_observers(&FilenameField::onSetEditFullPath); });
  }
};
  ...
  , m_askOverwrite(true)
{
  ...
  FilenameFieldObserver::instance().add_observer(this);
}

FilenameField::~FilenameField()
{
  FilenameFieldObserver::instance().remove_observer(this);
}
