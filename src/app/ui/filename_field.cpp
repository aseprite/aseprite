...
const std::string FilenameField::updatedFilename() const
{
  if (!m_entry) {
    ...
    return (m_showFullPath ? m_entry->text():
                             base::join_path(m_path, m_entry->text()));
  }
}
...
void FilenameField::onBrowse()
{
  ...
  ui::MenuItem relative(
    Strings::select_file_relative_path(m_pathBase));
...
void FilenameField::setFilename(const std::string& pathAndFilename)
{
  if (m_showFullPath || base::get_file_path(m_docFilename).empty()) {
    ...
    m_file = base::get_relative_path(pathAndFilename,
      base::get_file_path(m_docFilename));
