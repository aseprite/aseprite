...
class CmdSerial : public doc::IdMapperIO,
                  public doc::SubObjectsIO {
public:
  enum Coding : uint8_t { Decoding = 0, Encoding = 1 };
  using IdsMap = std::map<doc::ObjectId, doc::ObjectId>;
  using ImagesMap = std::map<doc::ObjectId, doc::ImageRef>;
  using CelDataMap = std::map<doc::ObjectId, doc::CelDataRef>;
  ...
  bool beforeCurState() const { return m_beforeCurState; }
  ...
  // IdMapperIO impl
  doc::ObjectId mapId(doc::ObjectId id, doc::ObjectType type) const override;
...
protected:
  ...
  mutable IdsMap m_idsMap;
  ImagesMap m_images;
  CelDataMap m_celdatas;
...
public:
  ...
  void curStateMark() override;
  void serializeObjectId(doc::ObjectId& v) override;
  void operator()(bool& v) override;
...
private:
  int curChar() const { return m_chr; }
  int nextChar();
  void skipWhitespace();
