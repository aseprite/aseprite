...
void AppMenus::reload()
{
  ...
  {
    Menu* helpMenu =
      dynamic_cast<MenuItem*>(m_rootMenu->findItemById("help_menu"))->getSubmenu();

    auto item = helpMenu->findChild("enter_license_separator");
    if (item)
      delete item;

    item = helpMenu->findChild("enter_license");
    if (item)
      delete item;
