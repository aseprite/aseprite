...
function init(plugin)
  plugin:newFileFormat{
    name="lua",
    onsave=function(ev)
       if (ev.sprite.layers[1].name ~= "Test") then error() end
       ev.file:write("return true")
       return true
    end,
    extensions={"lua"}
  }
  ...
  if (base::is_file("./save_result.lua"))
    base::delete_file("./save_result.lua");
...
function init(plugin)
  plugin:newFileFormat{
    ...
    onsave=function(ev) end,
    onload=function(ev) end,
    extensions={"format0", "format6", "format2", "format7", "format8" }
  }
end
)" },
    { "test.format0", ""  }
  ...
  ctx->executeCommand(
    ...
    {
      { "filename", base::get_absolute_path(base::join_path(e2.path(), "test.format0")) },
  });
...
TEST(Extensions, CustomFormatsInvalidReturn)
{
  const ExtFiles extensionFiles = {
    { "package.json", R"(
{
  "name": "test-custom-formats-invalid-return",
  "displayName": "Custom Formats Invalid Return",
  "contributes": {
    "scripts":  "./script.lua"
  }
})" },
...
function init(plugin)
  ...
  const ExtTestHelper e("test-custom-formats-invalid-return", extensionFiles);
  INIT_EXTENSION_TEST();
...
TEST(Extensions, CustomFormatsInvalidArguments)
{
  const ExtFiles extensionFiles = {
    { "package.json", R"(
{
  "name": "test-custom-formats-invalid-arguments",
  "displayName": "Custom Formats Invalid Arguments",
  "contributes": {
    "scripts":  "./script.lua"
  }
})" },
    { "script.lua",   R"(
function init(plugin)
  -- Invalid variable type to extension(s)
  pcall(function()
    plugin:newFileFormat{
      name="invalid1",
      onload=function() end,
  ...
  const ExtTestHelper e("test-custom-formats-invalid-arguments", extensionFiles);
  INIT_EXTENSION_TEST();
